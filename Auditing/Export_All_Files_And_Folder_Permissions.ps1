<#
.SYNOPSIS
Generates a detailed SharePoint Online permissions report using PnP PowerShell.

.DESCRIPTION
This script connects to a specified SharePoint Online site and exports
permissions information to a CSV file. It retrieves permissions for:
- Site Collection
- Webs (Sites)
- Lists and Libraries
- List Items and Folders (if -ScanItemLevel is used)

The report includes object type, title, URL, whether permissions are unique,
users/groups, permission levels, and how permissions were granted.
SharePoint group memberships are expanded to list individual users.

.NOTES
- This script requires a registered PnP PowerShell app. The ClientID
  of the registered app must be specified in the $ClientID parameter.

- Switches can be modified at the bottom of the script. Parameters for Site URL,
  ClientID, and output report file are also defined at the bottom of the script.
...
- Command 1 (Without -Recursive):
  Targets only the Root Site of the URL provided. Reports permissions of the
  top-level site and, when used with -ScanItemLevel, includes lists, libraries,
  and folders within that site. Sub-sites are not scanned.

- Command 2 (With -Recursive):
  Performs a full crawl starting from the Root Site. Recursively scans all
  sub-sites and their lists, libraries, folders, and items. Permissions for
  all objects in the site collection are included.

.EXAMPLE
# Scan only the root site and all lists/folders
...
.EXAMPLE
# Scan the root site and all sub-sites recursively
Generate-PnPSitePermissionRpt `
    -SiteURL "https://yourtenant.sharepoint.com/sites/YourSite" `
    -ReportFile "C:\Temp\YourReport.csv" `
    -Recursive `
...
.EXAMPLE
# Include inherited permissions in the report
Generate-PnPSitePermissionRpt `
    -SiteURL "https://yourtenant.sharepoint.com/sites/YourSite" `
    -ReportFile "C:\Temp\YourReport.csv" `
    -Recursive `
...
Object           Title                    URL                                                                  HasUniquePermissions Users                  Type                 Permissions               GrantedThrough
------           -----                    ---                                                                  -------------------- -----                  ----                 -----------               --------------
Site Collection  Marketing Team Site      https://crescent.sharepoint.com/sites/marketing                      TRUE                 John Doe               SecurityGroup        Web-Only Limited Access   Direct Permissions
File             Document1.docx           https://crescent.sharepoint.com/sites/marketing/Shared Documents/... FALSE                Jane Smith             SharePointGroup      Full Control              SharePoint Group: Marketing Owners
File             Document2.xlsx           https://crescent.sharepoint.com/sites/marketing/Shared Documents/... FALSE                Alex Johnson           SharePointGroup      Edit                      SharePoint Group: Marketing Members
File             Presentation.pptx        https://crescent.sharepoint.com/sites/marketing/Shared Documents/... FALSE                Emily Davis            User                 Contribute                Direct Permissions
File             Report.pdf               https://crescent.sharepoint.com/sites/marketing/Shared Documents/... FALSE                Michael Brown          SharePointGroup      Full Control              SharePoint Group: Marketing Owners
File             Notes.txt                https://crescent.sharepoint.com/sites/marketing/Shared Documents/... FALSE                Sarah Wilson           User                 Contribute                Direct Permissions
#>
